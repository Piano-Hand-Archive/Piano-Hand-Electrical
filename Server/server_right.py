...
# MATCHED TO ESP32
CHAR_UUID = "19b10002-e8f2-537e-4f6c-d104768a1215" # Difference
FILE_NAME = "test.txt"
...
async def play_song(client, lines):
    ...
    for line in lines:
        parts = line.split(":")
        try:
            # Assumes format: timestamp:command:value
            target_time = float(parts[0]) / 1000.0 # Convert ms to seconds
        except:
            continue
...
async def main():
    ...
    device = await BleakScanner.find_device_by_name("ESP32-Piano-Right") # Difference
